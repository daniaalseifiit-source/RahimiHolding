...
SN 170 offers stability and solvency that make it ideal for demanding industrial and automotive applications. Full technical specifications and Certificate of Analysis (COA) are available upon request, ensuring transparency and quality assurance for global partners.`
      ...
      fa: {
        ...
        description:
          "روغن پایه باکیفیت گروه I برای ترکیب‌بندی روانکار؛ مورد اعتماد بازارهای دول خلیج و جهانی برای کاربردهای صنعتی و خودرویی.",
        content:
          `روغن پایه SN 170 یک <strong>روغن پارافینی گروه I</strong> با <strong>حلّالیت عالی</strong> و <strong>پایداری اکسیداسیون برتر</strong> است. این محصول به‌عنوان ماده‌ی پایه در فرمولاسیون‌های متنوع روانکار در بازارهای دول خلیج و بین‌المللی شناخته می‌شود.
...
- ترکیب‌بندی روغن‌های موتور سواری و سنگین
- فرمولاسیون سیالات فلزکاری و روغن‌های فرآیندی

<strong>مزایا فنی:</strong>
پایداری و حلّالیت این روغن آن را برای کاربردهای صنعتی و خودرویی مناسب می‌سازد. مشخصات فنی کامل و گواهی تجزیه (COA) بنا به درخواست ارائه می‌شود تا شفافیت و تضمین کیفیت برای شرکای جهانی فراهم گردد.`
      },
      ar: {
        title: "زيت أساس SN 170 – مجموعة I ممتاز",
        description:
...
SN 500 ensures reliable viscosity behavior, oxidation resistance, and compatibility with additive packages. Full technical specifications and COA are available upon request for audit-ready procurement.`
      ...
      fa: {
        ...
        content:
          `SN 500 یک <strong>روغن پارافینی سنگین</strong> با <strong>استحکام فیلم بالا</strong> و <strong>مقاومت در برابر تخریب حرارتی</strong> است. <strong>شاخص ویسکوزیته بالا</strong> و <strong>حلّالیت عالی</strong>، ترکیب‌بندی پایدار برای فرمولاسیون‌های پیچیده را فراهم می‌کند.
...
رفتار ویسکوزیته قابل‌اعتماد، مقاومت اکسیداسیون و سازگاری با بسته‌های افزودنی را تضمین می‌کند. مشخصات فنی و گواهی تجزیه (COA) بنا به درخواست ارائه می‌شود.`
      },
      ar: {
...
سلوك لزوجة موثوق، مقاومة للأكسدة وتوافق مع حزم الإضافات. المواصفات الفنية وشهادة التحليل (COA) متاحة عند الطلب.`
      }
    }
  },
...
// Backward compatibility exports for translations.ts
export const PRODUCTS = getProductsByLocale('en');
export const BLOG_POSTS = getBlogPostsByLocale('en');
